...
function generator(adj, noun) {
  "use strict";// makes code more secure
...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
   ...
   // determineDx func was removed since it is not used to change Pizza size
...
function updatePositions() {
  // updatePositions is def facto a loop, we want to reduce activity in loops
  frame++;
  window.performance.mark("mark_start_frame");
  var items = document.getElementsByClassName('mover');
  //ScrollValue is calculated out of loop 
  var scrollValue = document.body.scrollTop / 1250;
  var phase = [];
  // phase have five values
  for (i = 0; i < 5; i++) {
    phase[i] = Math.sin(scrollValue) + (i % 5);
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Reduce the number of DOM elements (moving background pizzas)
